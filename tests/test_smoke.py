...
def test_detect_anomalies_explicit_methods():
    data = np.array([1.0, 2.0, 1.5, 100.0, 1.8, 2.1])
    for m in ("zscore", "iqr"):
        result = sz.detect_anomalies(data, method=m)
...
def test_text_classify():
    ...
    result = sz.text_classify(texts, labels, cv=2)
...
def test_cluster():
    ...
    result = sz.cluster(X, k_range=(2, 8))
